...
- Local development: run `mh` before `pipenv install --dev` (see [SRE Standards](./sre_standards.md#codeartifact-local-authentication))
...
**Transitional state:** Until migration completes, some repos still install `api_utils` from GitHub with `GITHUB_TOKEN`/`GH_PAT` in Docker builds. Follow the rollout in [DEPENDENCY_MOVE.md](https://github.com/mentor-forge/mentorhub_cloudformation/blob/main/docs/specifications/DEPENDENCY_MOVE.md).
