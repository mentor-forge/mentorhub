...
**Registration:** Squarespace form → Google Sheet → script → special `POST Profile` → AWS Cognito AdminCreateUser with custom claims. Not Hosted UI self-signup for claims. Cognito Forms is optional alternate tooling only.
...
| — | Squarespace → Sheet → POST Profile (R2) | Primary onboarding; optional Cognito Forms in `Research/cognito_forms/` |
...
| F-D21 | Extend **Customer** + confirm **Profile** | Customer today: `_id`, `name`, `description`, `created`, `saved`, `status`. Profile already: `full_name`, `email`, `email_verified`, `customer_id`, `roles`, … — add only gaps from Squarespace/Sheet / Admin research; **no** `gdpr_*` |
...
| F-CA05 | Special POST Profile + AWS Cognito Admin claims | Google Sheet script after Squarespace registration |
...
**Do not file:** “Cognito sign-in / sign-up integration” replacing login screens (redirect exists); “post-auth company onboarding” that duplicates Squarespace registration.
...
1. Do This First research (claims, Squarespace/Sheet handoff, Stripe)  
2. E0 cleanup (SPA → API → Data drops → F-W09 Coordinator removal)  
3. E1 Squarespace → Sheet provisioning  
